...
2. **Text to Speech (TTS)** - Powered by `ElevenLabs`.
...
```bash
cd /Users/leonard/melon-ai-learning-app/src/melon-ai-backend
...
### 2. Generate Content (LLM Module)
Generates educational lesson content (Multiple Choice Questions) based on a topic. Optionally, it can utilize the uploaded context (RAG) using the `file_id`.
...
```
*Note: Local HuggingFace image generation is disabled in this backend setup.*
